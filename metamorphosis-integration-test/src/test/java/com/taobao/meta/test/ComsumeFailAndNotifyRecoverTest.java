...
import com.taobao.metamorphosis.client.extension.ExtMessageSessionFactory;
import com.taobao.metamorphosis.client.extension.ExtMetaMessageSessionFactory;
...
public class ComsumeFailAndNotifyRecoverTest extends BaseMetaTest {
    ...
    public void setUp() throws Exception {
        ...
        this.sessionFactory = new ExtMetaMessageSessionFactory(metaClientConfig);
    ...
    public void sendConsume() throws Exception {
        ...
        final ConsumerConfig consumerConfig = new ConsumerConfig("notify-recover-group11");
        consumerConfig.setMaxFetchRetries(5);
        this.consumer =
                ((ExtMessageSessionFactory) this.sessionFactory).createConsumer(consumerConfig,
                    ConsumerRecoverType.NOTIFY);
        ...
        try {
            ...
            this.consumer.subscribe(topic, 1024 * 1024, new MessageListener() {
                ...
                public void recieveMessages(final Message messages) {
                    ComsumeFailAndNotifyRecoverTest.this.queue.add(messages);
                    // 第一次接收到抛异常,retry 5+1次均抛异常之后消息才进入recover,recover一次处理成功
                    // 共接收这条消息8次
                    // 加上第二条消息,队列里一共有9个消息
                    if (Arrays.equals(messages.getData(), "hello1".getBytes())
