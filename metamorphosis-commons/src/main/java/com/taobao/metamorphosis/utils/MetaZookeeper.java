...
public class MetaZookeeper {
    ...
    public Map<Integer, String> getMasterBrokersByTopic(final String topic) {
        ...
        final List<String> brokerIds = ZkUtils.getChildren(this.zkClient, this.brokerTopicsPath + "/" + topic);
        if (brokerIds == null) {
            return ret;
        }
    ...
    public Map<String, List<Partition>> getPartitionsForTopicsFromMaster(final Collection<String> topics) {
        ...
        for (final String topic : topics) {
            ...
            final List<String> brokers = ZkUtils.getChildren(this.zkClient, this.brokerTopicsPath + "/" + topic);
            for (final String broker : brokers) {
                final String[] brokerStrs = StringUtils.split(broker, "-");
                if (brokerStrs != null && brokerStrs.length == 2 && brokerStrs[1].equals("m")) {
                    final int nParts =
                            Integer.parseInt(ZkUtils.readData(this.zkClient, this.brokerTopicsPath + "/" + topic + "/"
                                    + broker));
    ...
    public Set<String> getTopicsByBrokerIdFromMaster(final int brokerId) {
        ...
        final List<String> allTopics = ZkUtils.getChildren(this.zkClient, this.brokerTopicsPath);
        ...
        if (topics != null) {
            for (final String topic : topics) {
                ...
                final String dataString =
                        ZkUtils.readDataMaybeNull(this.zkClient, this.brokerTopicsPathOf(topic, brokerId, -1));
                if (StringUtils.isBlank(dataString)) {
                    continue;
                }
                final int nParts = Integer.parseInt(dataString);
                for (int part = 0; part < nParts; part++) {
                    if (partList == null) {
                        partList = new ArrayList<Partition>();
                    }
     ...
     * @return
     */
    public Map<String, List<String>> getPartitionStringsForTopics(final Collection<String> topics) {
        final Map<String, List<String>> ret = new HashMap<String, List<String>>();
        for (final String topic : topics) {
